...
def calculate_vRES_demand(output_dir : str, output_file : str, 
    ...
    - elec_loss_rate: str - Path to the file recording the share of
        ...
        net import.
    - hydro_nuclear_prod: str - Yearly aggregated production of hydropower and nuclear 
    ...
    df_elec_decarb_tot = df_elec_decarb.mul(population[population.columns[0]], axis=0) * 1e-3 # switch from GJ to TJ
    # Sum up across all sectors
    df_elec_decarb_sum = df_elec_decarb_tot.sum(axis=1).to_frame(name='total')
    # Get hydro and nuclear production
    df_hydro_nuclear_prod = pd.read_csv(hydro_nuclear_prod, index_col=0)
    # Total vRES production needed is total decarbonised production minus hydro and nuclear production
    df_vres_demand = (df_elec_decarb_sum['total'] - df_hydro_nuclear_prod[df_hydro_nuclear_prod.columns[0]]).clip(lower=0)
