...
def get_elec_TnD_loss_rate(
    ...
    for filepath in list(inputs):
        ...
        matches = df.loc[(df['flow']=='EHINDPROD') & (df['product']=='ELECTR'), "value"]
        tot_prod = matches.iloc[0] if not matches.empty else 0
        # Get the net import
        matches_import = df.loc[(df['flow']=='IMPORTS') & (df['product']=='ELECTR'), "value"]
        imports = matches_import.iloc[0] if not matches_import.empty else 0
        matches_export = df.loc[(df['flow']=='EXPORTS') & (df['product']=='ELECTR'), "value"]
        ...
        matches_loss = df[(df['flow']=='DISTLOSS') & (df['product']=='ELECTR')]['value']
