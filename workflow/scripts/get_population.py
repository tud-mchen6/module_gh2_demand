...
def get_population(
    ...
    - output_dir: str - Path to the directory to store the output file.
    ...
    df_all = df_all.pivot(columns='Time', index='ISO3', values='PopTotal') * 1e3 # switch unit
    ...
    df_all[[population_ref_year]].round(0).rename(columns={population_ref_year:'population'}).to_csv(population_output)
