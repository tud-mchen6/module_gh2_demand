...
def get_target_elec_decarb(
    ...
    - use_historical: bool - Whether to directly use historical values in the 
        reference year.
    - elec_decarb_rel: float - Decarbonisation of electricity relative to historical ones 
    ...
    sector = 'ELEC_DECARB'
    sectors = ['TOTIND', 'TOTTRANS', 'RESIDENT', 'COMMPUB', 'AGRICULT', 'FISHING', 'ONONSPEC', 'NONENUSE']
            ...
            for country in list(country_level_overwrite_df.index):
                ...
                if pd.notna(abs) and pd.notna(rel):
                    ...
                    if reference_df.at[country, sector] > abs:
                        print("Input warning: given electricity decarbonisation level lower than historical value"
            ...
            if 'sectorShare' in file_name:
                ...
                if country in countries:
                    ...
                    for sector in list(country_sector_specific_overwrite.index):
                        abs = country_sector_specific_overwrite.loc[sector, 'elec_decarb_abs']
                        rel = country_sector_specific_overwrite.loc[sector, 'elec_decarb_rel']
                        if pd.notna(abs) and pd.notna(rel):
                            print("Input error: relative values and absolute values cannot be given at the same time for " \
                                "electricity decarbonisation level.")
                        elif pd.notna(abs):
