...
def get_elec_heat_balances_df(country, year): # if given an invalid string, this function just returns every country's value
...
def download_elec_heat_balances(country_codes_path: str, output_dir : str, year: int):
    """
    ...
    for i, country in country_codes.iterrows():
        if type(country['IEA']) is not float:
            get_elec_heat_balances_df(country['IEA'], year).to_csv(output_dir + 'IEA_elec_heat_balances_'+country['ISO3'] +
                                                               '_' + str(year) + '.csv', index=False)
