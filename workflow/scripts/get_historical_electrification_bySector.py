...
def process_countries_electrification_bySector(inputs : str, countries, output_dir : str, output_file : str):
    """
    ...
    for country, file_path in zip(countries, inputs):
        ...
        df_electricity = df[(df['flow'].isin(list_sectors)) & (df['product'] == 'ELECTR')] \
                        .pivot(index='short', columns='flow', values='value')
        # Step 2. Get TFC in each sector
        df_TFC = df[(df['flow'].isin(list_sectors)) & (df['product'] == 'TOTAL')] \
                        .pivot(index='short', columns='flow', values='value')
